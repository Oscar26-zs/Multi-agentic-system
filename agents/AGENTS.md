...
**Modo propuesta (`--propuesta`):** cuando `state["proposal_mode"]` es `True`, este agente **no aplica** los cambios contra el repo real. Sigue explorando en solo lectura (MCP de lectura) para contextualizar, pero el plan (`DeveloperPlan`) se deriva a diffs con `agents/mcp_tools.py::plan_to_changes()` (puro Python, sin `create_file`/`update_file`) y se deja en `implementation["diff"]` para que `app.py` lo vuelque al archivo de propuesta. Lo mismo hace `testing_agent.py` con sus casos de test (que además no corre `run_tests`). Ver `README.md` (sección "Modo propuesta").
...
- **`MAX_TOOL_ITERATIONS = 8` como salvavidas, no como fallo.** Si se agota, el agente igual entrega un reporte coherente con lo que realmente pasó (aunque sea parcial), en vez de lanzar una excepción y tirar todo el progreso del ciclo.
